...
_PIN_NAME = {"power": "PWR"}   # display override; other kinds use kind.upper()
...
def sheet_pins_for(block: Block, connections: list[Connection]) -> list[Pin]:
    ...
    if not touching:
        return []
    touching.sort(key=lambda t: (
        _TYPE_ORDER.get(t[0].type, 9), t[1],
        t[0].target if t[1] == "output" else t[0].source,
    ))
    # Pass 1: per-kind direction sets (so the FIRST pin already knows it's bidi).
    kind_dirs: dict[str, set[str]] = {}
    for c, direction in touching:
        kind_dirs.setdefault(c.type, set()).add(direction)
    # Pass 2: emit.
    pins: list[Pin] = []
    counts: dict[str, int] = {}
    for c, direction in touching:
        ...
        counts[kind] = n
        base = _PIN_NAME.get(kind, kind.upper())
        name = base if n == 1 else f"{base}{n}"
        shape = "bidirectional" if {"input", "output"} <= kind_dirs[kind] else direction
